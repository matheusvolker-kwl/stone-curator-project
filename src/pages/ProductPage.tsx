...
export default function ProductPage() {
    ...
    return (
      <div className="container-western py-32">
        <div className="grid md:grid-cols-2 gap-12">
          <div className="aspect-square bg-western-green-mid animate-pulse" />
          <div className="space-y-4">
            <div className="h-12 w-2/3 bg-western-green-mid animate-pulse" />
            <div className="h-4 w-1/3 bg-western-green-mid animate-pulse" />
          </div>
        </div>
      </div>
    );
  ...
  if (!product) {
    return (
      <div className="container-western py-32 text-center">
        <h1 className="font-display text-4xl text-western-cream">Peça não encontrada</h1>
  ...
  return (
    <div className="container-western py-12 md:py-20">
      <Link
        to={collection ? `/colecoes/${collection.handle}` : "/colecoes"}
      ...
      <div className="grid md:grid-cols-2 gap-10 lg:gap-16 items-start">
        {/* Gallery */}
        <div>
          <div className="frame-gallery aspect-square overflow-hidden">
            {images[activeImage] && (
              <img
            ...
            <div className="flex gap-3 mt-4 overflow-x-auto">
              {images.map((img, idx) => (
                <button
                  ...
                  className={`frame-gallery w-20 h-20 flex-shrink-0 transition-opacity ${
        ...
        <div className="md:py-4">
          {collection && (
            <p className="text-eyebrow mb-5">{collection.title}</p>
          )}
          <h1 className="font-display text-4xl md:text-5xl leading-tight text-western-cream">
            {product.title}
          </h1>
          {sku && (
            <p className="text-spec text-western-cream-muted mt-3">SKU {sku}</p>
          )}

          {product.description && (
            <p className="mt-8 text-western-cream-muted leading-relaxed">
              {product.description}
            </p>
          )}
              ...
              <div key={option.name} className="mt-10">
                ...
                <div className="flex flex-wrap gap-3">
                  {option.values.map((val) => {
                    const selected =
                      (activeOptions[option.name] ?? variant?.selectedOptions.find((o) => o.name === option.name)?.value) === val;
                    return (
                      <button
                        key={val}
                        onClick={() =>
                        ...
                        className={`px-5 py-2.5 font-mono text-xs uppercase tracking-[0.2em] border transition-all duration-300 ${
                          selected
                            ? "border-western-gold text-western-gold-soft bg-western-gold/5"
                            : "border-western-gold/25 text-western-cream hover:border-western-gold/60"
                        }`}
                      >
                        {val}
                      </button>
          ...
          <div className="mt-12 pt-8 border-t border-western-gold/15">
            <div className="flex items-baseline justify-between mb-6">
              ...
              <span className="font-display text-3xl text-western-cream">
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <Button
                ...
                className="flex-1 h-12 bg-western-gold text-western-green-deep hover:bg-western-gold-soft font-mono text-xs uppercase tracking-[0.25em] rounded-none"
              >
                {isLoadingCart ? (
          ...
          {/* 3D Model */}
          <div className="mt-10 p-6 border border-western-gold/20 bg-western-green-mid/40">
            <p className="text-eyebrow mb-2">Modelo 3D — SketchUp</p>
            <p className="text-spec text-western-cream-muted mb-4">
            ...
            <a
              ...
              className="inline-flex items-center gap-2 link-underline text-western-gold-soft font-mono text-xs uppercase tracking-[0.2em]"
            >
          ...
          <div className="mt-10">
            ...
            <dl className="space-y-2 text-spec">
              <div className="flex justify-between border-b border-western-gold/10 py-2">
                <dt className="text-western-cream-muted">Material</dt>
                <dd>Composto mineral de alta resistência</dd>
              </div>
              <div className="flex justify-between border-b border-western-gold/10 py-2">
                <dt className="text-western-cream-muted">Garantia</dt>
                <dd>1 ano</dd>
              </div>
              <div className="flex justify-between border-b border-western-gold/10 py-2">
                <dt className="text-western-cream-muted">Origem</dt>
